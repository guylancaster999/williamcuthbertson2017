...
function mnuClass()
{
 echo ' class="col-sm-3 col-md-2 col-lg-2" ';
   return;
}
function mainClass()
{
 echo ' class="col-sm-9 col-md-10 col-lg-10" ';
  return  ;
}
function shead1($t)
{
 	$ret='<h1 style="text-align:center">William Cuthbertson</h1>
		<a id="top"></a>
	      <header><h2 style="text-align:center">'.$t.'</h2></header>';
	return $ret;
} 
...
function abutton($url,$ttl,$scrn)
{
	...
	$ret= '<a href="'.$tgt.'" title="'.$ttl.'"   style="color:  white; 
	padding-left:	10px; 
	padding-left:	8px; 
	padding-right:	4px; 
 
	width:100%;
	padding-bottom:	8px;!important">'.$ttl.' </a>&nbsp;&nbsp;<br/>';
	return $ret;
}
function menu($lan="de",$scrn="",$pgen="index_e.php",$pgde="index.php",$pgch="index_ch.php")
{
  ...
  $ret.=abutton('news_ch.php','新闻',$scrn);
  ...
  case 'de':
   ...
   $ret.=abutton('concert.php','Konzerte',$scrn);
    $ret.=abutton('news.php','Aktuelles',$scrn);    
...
$ret.='</div>';
 $ret.="<form action='search.php' method='post'>
	...
	style='margin-left:4px;width:80px;background-color:white;color:black;'  required />
</form> <br/>";
	...
	switch($lan)
	{
		case "de":
		   $ret.='<form action="http://cuthbertson.de/m.index.php">
				&nbsp;&nbsp;<input type="submit" value="Mobile" class="mnumbl" />
				</form>';
		break;
		case "en":
		...
		break;
		case "ch":	
			$ret.='<form action="http://cuthbertson.de/m.index_ch.php">
					&nbsp;&nbsp;<input type="submit" value="移动电话"  class="mnumbl">
					</form>';
		}
 		$ret.='</nav>';
	$ret.="</div>";
...
function head($lan="de",$scrn="",$url="",$extrakeys="")
{
$ret= '<!DOCTYPE html>';
   switch ($lan)
	{
 ...
 switch ($lan)
	{
	   case "de":
		...
		$ret.='<meta name="keywords" content="'.$scrn.',Piano, Klavier, Chopin, Beethoven, William, Cuthbertson, Waldkirch, Konzerte'.$extrakeys.'" />';
	...
	$ret.='<meta name="google-site-verification" content="bZ6m0q-fUDJPfnLQW2baPJa3xthonO5H0zyh5S5p1QU" />
	    </head>';
    return  $ret;
 }	 
...
function displayPic($i,$fromUrl,$lan,$gpics)
{
	...
	$cpy		= "";
	if (isset($y["cpy"]))$cpy	= removeSp($y["cpy"]);
...
function displaySketch($i,$fromUrl,$lan,$gpics)
{
 	$x 			= json_decode($gpics,true);
	$sz			= count($x["gallery"]);
	$y 			= $x["gallery"][$i];
   ...
   $ret		.='" ';
    $ret.=exp_title($picTtl,$lan);
    $ret.='>';
    $ret.= '<img height="200" src="img/'.$picSmall.'" alt="'.$picTtl.'" class="img-responsive img-rounded bordered " />';
    $ret.='</a>';
    print $ret;
	return;
}
...
{
   $photoTtl = removeSp($photoTtl);
   $ret ='<figure><a href="fotoLarge.php';
   $ret.='?lan='.$lan;
   $ret.='&amp;album='.$album;
...
{
	$photoTtl=removeSp($photoTtl);
    $ret ='<figure><a href="fotoLarge1.php';
	$ret.='?lan='.$lan;
	$ret.='&amp;album='.$album;
	$ret.='&amp;photoTtl='.$photoTtl;
	$ret.='&amp;fromUrl='.$fromUrl;
